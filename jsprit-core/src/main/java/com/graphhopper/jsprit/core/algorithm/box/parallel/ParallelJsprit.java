...
public class ParallelJsprit {
    ...
    public static ParallelVehicleRoutingAlgorithm createAlgorithm(VehicleRoutingProblem vehicleRoutingProblem, String id, RedissonClient redisson) {
        ...
        return ParallelJsprit.Builder.newInstance(vehicleRoutingProblem)
                                     .addId(id)
                                     .addRedisson(redisson)
                                     .buildAlgorithm();
    }
    ...
    public static class Builder {
        ...
        public Builder addRedisson(RedissonClient redisson) {
        ...
        public ParallelVehicleRoutingAlgorithm buildAlgorithm() {
            return new ParallelJsprit(this).create(vrp, id, redisson);
        }
    ...
    private ParallelJsprit(Builder builder) {
    ...
    private ParallelVehicleRoutingAlgorithm create(final VehicleRoutingProblem vrp, String id, RedissonClient redisson) {
        ini(vrp);
        ...
        ParallelIterationStartsListener schrimpfThreshold = null;
        if(acceptor == null) {
        ...
        radialRegret.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.RADIAL_REGRET.toString(), regret, radial, id, redisson)));

        SearchStrategy radialBest = new SearchStrategy(Strategy.RADIAL_BEST.toString(), new SelectBest(), acceptor, objectiveFunction);
        radialBest.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.RADIAL_BEST.toString(), best, radial, id, redisson)));

        SearchStrategy randomBest = new SearchStrategy(Strategy.RANDOM_BEST.toString(), new SelectBest(), acceptor, objectiveFunction);
        randomBest.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.RANDOM_BEST.toString(), best, random_for_best, id, redisson)));

        SearchStrategy randomRegret = new SearchStrategy(Strategy.RANDOM_REGRET.toString(), new SelectBest(), acceptor, objectiveFunction);
        randomRegret.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.RANDOM_REGRET.toString(), regret, random_for_regret, id, redisson)));

        SearchStrategy worstRegret = new SearchStrategy(Strategy.WORST_REGRET.toString(), new SelectBest(), acceptor, objectiveFunction);
        worstRegret.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.WORST_REGRET.toString(), regret, worst, id, redisson)));

        SearchStrategy worstBest = new SearchStrategy(Strategy.WORST_BEST.toString(), new SelectBest(), acceptor, objectiveFunction);
        worstBest.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.WORST_BEST.toString(), best, worst, id, redisson)));
        ...
        clustersRegret.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.CLUSTER_REGRET.toString(), regret, clusters, id, redisson)));
        ...
        clustersBest.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.CLUSTER_BEST.toString(), best, clusters, id, redisson)));

        SearchStrategy stringRegret = new SearchStrategy(Strategy.STRING_REGRET.toString(), new SelectBest(), acceptor, objectiveFunction);
        stringRegret.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.STRING_REGRET.toString(), regret, stringRuin, id, redisson)));

        SearchStrategy stringBest = new SearchStrategy(Strategy.STRING_BEST.toString(), new SelectBest(), acceptor, objectiveFunction);
        stringBest.addModule(configureModule(new ParallelRuinAndRecreateModule(Strategy.STRING_BEST.toString(), best, stringRuin, id, redisson)));

        PrettyParallelAlgorithmBuilder prettyBuilder = PrettyParallelAlgorithmBuilder.newInstance(vrp, vehicleFleetManager, stateManager, constraintManager);
        ...
        ParallelVehicleRoutingAlgorithm vra = prettyBuilder.build(id, redisson);
    ...
    private SearchStrategyModule configureModule(ParallelRuinAndRecreateModule ruinAndRecreateModule) {
        ...
        ruinAndRecreateModule.setProportionOfUnassignedJobsToBeReinserted(Double.valueOf(properties.getProperty(
                Parameter.PROPORTION_UNASSIGNED.toString())));
    ...
    private void handleExecutorShutdown(ParallelVehicleRoutingAlgorithm vra) {
