...
class SwitchVehicleListener implements EventListener {
    ...
    private static final Logger logger = Logger.getLogger(SwitchVehicleListener.class.getName());
    ...
    public void inform(Event event) {
        if (event instanceof SwitchVehicle) {
            ...
            if (vehiclesDifferent((SwitchVehicle) event)) {
                logger.info(String.format("switch vehicle (%s to %s)",((SwitchVehicle) event).getRoute().getVehicle().getId(),((SwitchVehicle) event).getVehicle().getId()));
                ...
                if (aBreak != null) {
                    ...
                    if (removed) logger.info("remove " + aBreak.getId());
                }
            }
